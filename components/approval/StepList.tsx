  ...
  return (
    <Card>
      <div className="px-5 pt-4 pb-2">
        <h3 className="text-[15px] font-semibold">Approval chain</h3>
        <p className="text-[12px] text-muted">Steps are decided by routing, never picked by the rep.</p>
      </div>
      <div className="px-5 pb-5">
        {approvals.length === 0 ? (
          <div className="rounded-[8px] border border-dashed border-border-strong px-4 py-3 text-[13px]">
            {status === "DRAFT" ? (
              <>
          ...
          <ol className="flex flex-col gap-2">
            {approvals.map((a) => (
              <li key={a.id} className="flex items-start gap-3 rounded-[8px] border border-border px-3 py-2.5">
                <span className="num mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-raised text-[12px]">{a.step}</span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-[13px] font-medium">{stepLabel(a.role as "SALES_MANAGER" | "FINANCE")}</p>
                  ...
                  {a.approver ? (
                    ...
                    <p className="mt-0.5 text-[12px] text-muted">Waiting</p>
                  )}
                  {a.reason ? <p className="mt-1 text-[12px]">“{a.reason}”</p> : null}
                </div>
              </li>
            ))}
          </ol>
