    ...
    <div className="grid grid-cols-12 gap-5">
      ...
      <div className="col-span-12 flex flex-col gap-5 xl:col-span-5">
        ...
        {canAct && pending ? (
          ...
          <Card className="p-4 text-[14px] text-muted">You submitted this quotation, so the decision has to come from someone else.</Card>
        ) : pending && !roleOk ? (
...
function Stat({ label, value, tone }: { label: string; value: string; tone: "money" | "warn" | "danger" }) {
  ...
  return (
    <div className="card px-4 py-3">
      ...
      <p className={`display num mt-0.5 text-[26px] font-semibold ${cls}`}>{value}</p>
    </div>
  );
}
